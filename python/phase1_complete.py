...
DATA_DIR = "./data"
RAW_DATA_FILE = f"{DATA_DIR}/openfoodfacts_raw.csv"
CLEANED_DATA_FILE = f"{DATA_DIR}/openfoodfacts_cleaned.csv"
...
def create_database_schema(engine):
    """
    Create PostgreSQL schema with all tables and constraints
    """
    print("=" * 80)
    ...
    -- Table 2: brands (normalized)
    CREATE TABLE brands (
        brand_id SERIAL PRIMARY KEY,
        brand_name VARCHAR(200) UNIQUE NOT NULL
    );
    
    -- Table 3: product_brands (many-to-many junction)
    ...
    -- Table 4: categories (normalized)
    CREATE TABLE categories (
        category_id SERIAL PRIMARY KEY,
        category_name VARCHAR(200) UNIQUE NOT NULL
    );
    
    -- Table 5: product_categories (many-to-many junction)
    ...
    -- Table 6: countries (normalized)
    CREATE TABLE countries (
        country_id SERIAL PRIMARY KEY,
        country_name VARCHAR(100) UNIQUE NOT NULL
    );
    
    -- Table 7: product_countries (many-to-many junction)
    ...
    -- Table 8: labels (normalized)
    CREATE TABLE labels (
        label_id SERIAL PRIMARY KEY,
        label_name VARCHAR(200) UNIQUE NOT NULL
    );
    
    -- Table 9: product_labels (many-to-many junction)
    ...
    print("  - Primary keys, foreign keys, and constraints applied")
